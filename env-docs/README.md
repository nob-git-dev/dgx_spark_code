...
mkdir -p ~/docs
cp -r env-docs/* ~/docs/
cd ~/docs && git init && git add -A && git commit -m "init: 環境ドキュメント初期化"
```
...
cp ~/docs/claude/gx10-sync-env.md ~/.claude/commands/
```
...
~/docs/snapshots/ に date, hostname, uname -a, nvidia-smi 等の出力を保存
```
